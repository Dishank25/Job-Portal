...
class _SignupPageYourSkillsState extends State<SignupPageYourSkills> {
  ...
  void didChangeDependencies() {
    ...
    // Load all domain
    context.read<SkillBloc>().add(const LoadDomains());
    // Re-fetch sub-skills for previously selected domains
    for (final domain in selectedDomains) {
      if (!subSkillsMap.containsKey(domain)) {
        context.read<SkillBloc>().add(LoadSubSkills(domain));
      }
    }
                    ...
                    BlocListener<SkillBloc, SkillState>(
                      ...
                      listener: (context, state) {
                        if (state is SkillStateDomainLoaded) {
                          developer.log("🎯 Domains loaded: ${state.domainAllResponse.domains}");
                          developer.log("API Response: ${state.domainAllResponse}");
                          // if ()
                          setState(() {
                          ...
                          loadingSubSkills.remove(domain);

                          setState(() {
                            subSkillsMap[domain] = List<String>.from(data.skills); // always update
                          });
  ...
  Future<FormData> createParamsForSkill() async {
    final _prefs = sl<PreferencesManager>();
    // final user_id = '1';
    final user_id = _prefs.getuser_id() ?? '1';
