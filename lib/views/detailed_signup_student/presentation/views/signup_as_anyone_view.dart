...
class SignupAsAnyOne extends StatefulWidget {
  ...
  SignupAsAnyOne({
    ...
    required this.email, this.firstName,  this.lastName, this.phone,
  });
  @override
...
class _SignInPageUniversityStudentState extends State<SignupAsAnyOne> {
  ...
  void initState() {
    super.initState();
     ...
     // context.read<CollegeBloc>().add(FetchColleges());
     // first_nameController.text = 'first name';
    ...
    if (widget.firstName != null && widget.firstName!.isNotEmpty) {
      first_nameController.text = widget.firstName!;
    }
    if (widget.lastName != null && widget.lastName!.isNotEmpty) {
      surnameController.text = widget.lastName!;
    }
    if (widget.phone != null && widget.phone!.isNotEmpty) {
      phoneController.text = widget.phone!;
    }

    // Always set email (passed required)
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  BlocListener<DetailedSignupBloc, DetailedSignupState>(
                    listener: (context, state) {
                      if (state is DetailedSignupGetBasicUserInfoLoaded) {
                        ...
                        setState(() {
                          first_nameController.text = data.user.first_name;
                          ...
                          developer.log(" Locations from API: ${locations.locations}");
                        });
                        final emailMap = {'email': widget.email};
                        context
                  ...
                  // ),
                  CustomAutocomplete(
                    // key: ValueKey(cities.length), // Ensures rebuild when cities load
                    options: cities,
                    label: 'Current City',
                          ...
                          builder: (context, state) {
                            if (state
                              ...
                              final course = state.coursesListResponse;
                               developer
                                   .log('College list in ui : ${colleges.colleges}');
                              return Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(
  ...
  void fillDataIntoParams() {
    ...
    widget.params.addAll({
      ...
      DETAILEDPROFILEPARAMS.first_name.name: first_nameController.text,
