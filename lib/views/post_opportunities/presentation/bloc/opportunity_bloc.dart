...
class OpportunityBloc extends Bloc<OpportunityEvent, OpportunityState> {
  ...
  Future<void> _onCreateJobPost(
    ...
    try {
      emit(const OpportunityJobPostLoading()); // ✅ Correct loading state
      final response = await _createJobPostUsecase(params: event.params);
      emit(OpportunityJobPostLoaded(response.data!)); // ✅ Correct success state
    } catch (e) {
      emit(const OpportunityJobPostError()); // ✅ Correct error state
    }
  }
}
